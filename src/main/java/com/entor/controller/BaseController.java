...
public class BaseController<T> {
	...
	public void deleteMore(Class<T> cls,String ids) {
		...
		try {
			out = response.getWriter();
			baseservice.deleteMore(cls,ids);
			jo.put("state", 0);
			jo.put("msg", "成功删除记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void queryByPage(Class<T> cls,String page) {
		...
		int sp = 1;
		//总记录数
		int totals = baseservice.getTotals(cls);
		//每页记录数
		...
		List<Student> list = baseservice.queryByPage(Student.class, sp,pageSize);
		try {
